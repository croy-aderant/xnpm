...
function extractGitHubInfo(repoUrl: string): { owner: string; repo: string } | null {
...
async function checkMaintainers(
  githubInfo: { owner: string; repo: string },
  warnings: SecurityWarning[]
): Promise<void> {
  try {
    ...
    if (contributors.length > 0) {
      ...
      const contributorSet = new Set(recentCommits.map((c: any) => c.author?.login).filter(Boolean));
      
      if (contributorSet.size > 0 && recentCommits.length > 0) {
        // Check if there are contributors we haven't seen before
        const topContributors = contributors.slice(0, 10).map((c: any) => c.login);
        const newContributors = Array.from(contributorSet).filter(
          (login: string) => !topContributors.includes(login)
        );
...
async function checkNpmReleases(
  githubInfo: { owner: string; repo: string },
  packageName: string,
  currentVersion: string,
  latestVersion: string,
...
async function checkPackageJsonChanges(
  githubInfo: { owner: string; repo: string },
  currentVersion: string,
  latestVersion: string,
  warnings: SecurityWarning[]
...
function isMinifiedCode(message: string, files: any[]): boolean {
  ...
  if (files) {
    return files.some((f: any) => f.filename && /\.min\.(js|css)$/.test(f.filename));
  }
...
function isRandomPackageName(name: string): boolean {
...
function generateAnalysisResult(warnings: SecurityWarning[]): SecurityAnalysis {
...
async function fetchCommits(
  ...
  try {
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/compare/${fromTag}...${toTag}`,
      {
        headers: {
    ...
    return (response.data.commits || []).map((commit: any) => ({
      ...
      stats: response.data.files ? {
        total: response.data.files.reduce((sum: number, f: any) => sum + (f.additions || 0) + (f.deletions || 0), 0),
      } : { total: 0 },
...
async function fetchRecentCommits(
  ...
  try {
    ...
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/commits`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchTags(githubInfo: { owner: string; repo: string }): Promise<any[]> {
  try {
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/tags`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchContributors(githubInfo: { owner: string; repo: string }): Promise<any[]> {
  try {
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/contributors`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchFileContent(
  ...
  try {
    ...
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/contents/${filePath}?ref=${tag}`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchChangedFiles(
  ...
  try {
    ...
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/compare/${currentTag}...${latestTag}`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchWorkflows(githubInfo: { owner: string; repo: string }): Promise<any[]> {
  try {
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/actions/workflows`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchRepoInfo(githubInfo: { owner: string; repo: string }): Promise<any> {
  try {
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}`,
      {
        headers: {
          Accept: 'application/vnd.github.v3+json',
        },
...
async function fetchRecentActivity(githubInfo: { owner: string; repo: string }): Promise<any> {
  try {
    const response = await axios.get(
      `${githubApiUrl}/repos/${githubInfo.owner}/${githubInfo.repo}/commits`,
      {
...
async function fetchActivityBefore(
  githubInfo: { owner: string; repo: string },
  daysAgo: number
): Promise<any[]> {
  try {
